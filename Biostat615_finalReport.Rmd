...
For any two DNA fragments, $i$ and $j$, and a specific Alu family, $k$, the edge weight $e_i,j$ is calculated. This weight is the minimum of the boolean values $a_i$ and $a_j$ multiplied by the sum of the minimum number of Alu elements of family $k$ in both fragments, times the fragment contact probabilities $h_i,j$. $e_i,j$ is the edge weight between DNA fragment $i$ and $j$. $a_i$ is a boolean value indicating whether a window coincides with accessible chromatin for fragment $i$. $n_k,i$ is the number of Alu elements of the family $k$ in fragment $i$. $h_i,j$ is the contact probabilities between fragments $i$ and $j$. We can represent this as a matrix operation:
